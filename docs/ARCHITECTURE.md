...
- output formatting
- catalog-driven help and manifest rendering
- exit behavior
...
- zero-argument `aptitude` launches the guided wizard
- `install` with no advanced overrides may enter the guided install flow directly
- `sync` with no `--lock` may enter the guided sync flow directly
- explicit advanced command usage continues through the direct command path

CLI concept:

- the default human CLI is intentionally minimal and flow-led
- the guided path should lead the user through the next decision instead of requiring recall of the full flag surface
- common planning controls may be surfaced as constrained multi-option menus in the wizard while preserving the same underlying application contract
- explicit flags remain available for direct terminal use, automation, and debugging
