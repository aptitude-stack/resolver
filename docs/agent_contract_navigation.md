...
This repository currently contains the **Aptitude Client skeleton**, not a
...
- most folders are scaffolding with little or no implementation code yet
- `tests/` is currently empty
- `pyproject.toml` is intentionally minimal and should be treated as the source
  of truth for active dependencies

Agents must distinguish between:

- **current repository reality**: what actually exists in the filesystem
- **target architecture**: what the architecture documents describe as the
  intended direction
...
3. Ensure coverage requirements appropriate to the change
4. Keep tests aligned with current repository reality, not future diagrams
...
- `shared -> application | discovery | resolver`
...
Do not create modules that do not exist unless explicitly required.
...
3. Integrate discovery or resolver logic
